# MSBAi: Master of Science in Business Analytics - Online

**AI-First • Project-Based • Flexible • Affordable**

Welcome to the MSBAi curriculum design documentation. This site contains comprehensive program planning documents for the Master of Science in Business Analytics - Online degree, launching Fall 2026.

---

## 📋 Quick Start for Faculty & Admin

### Core Planning Documents

| Document | Purpose | For Whom |
|----------|---------|----------|
| **[Program Overview](CLAUDE.md)** | Program context, roadmap, session logs | Everyone (start here) |
| **[AI-First Strategy](MSBAI_AI_FIRST_STRATEGY.md)** | Strategic positioning, curriculum vision, pathways | Leadership, marketing, faculty leads |
| **[Guiding Principles & Constraints](MSBAI_GUIDING_PRINCIPLES_AND_CONSTRAINTS.md)** | 10 principles + hard/soft constraints | Faculty, instructional designers |
| **[Implementation Plan](IMPLEMENTATION_PLAN_AND_TASK_BREAKDOWN.md)** | 5-phase roadmap, timeline, resource allocation | Program manager, leadership |

### Curriculum Design

| Document | Purpose | For Whom |
|----------|---------|----------|
| **[Curriculum Structure](ONLINE_CURRICULUM_STRUCTURE.md)** | 8-week program timeline, term breakdown, TBD courses | Everyone |
| **[Course Adaptations](COURSE_ADAPTATIONS.md)** | Detailed 8-week syllabi (554, 513, 550, 557, 558, 576) | Faculty instructors, content developers |
| **[Electives Roadmap](ELECTIVES_ROADMAP.md)** | New electives, specialization tracks, certificates | Faculty, program director |
| **[Cohort Model](COHORT_MODEL.md)** | Student experience, studio sessions, community | Faculty, student services, program manager |

---

## 🎯 Program At A Glance

### Structure
- **Duration:** 4 terms (24-36 months depending on pace)
- **Total Credits:** 36
- **Format:** 8-week courses (vs. traditional 16-week)
- **Delivery:** Asynchronous-first + weekly synchronous studio sessions
- **Start:** Fall 2026

### Core Courses (16 credits)
1. **BADM 554** - Data Systems & Engineering (4 cr)
2. **BDI 513** - Data Storytelling (4 cr, split 2+2)
3. **FIN 550** - Analytics for Business (4 cr)
4. **BADM 557** - Business Intelligence with AI (4 cr)

### Advanced Courses (8 credits)
- **BADM 558** - Big Data Infrastructure (4 cr)
- **BADM 576** - Data Science & Machine Learning (4 cr)

### Electives & Capstone (12 credits)
- **Generative AI for Analytics** (2 cr) — NEW
- **Specialization Courses** (4 cr) — rotate by student interest
- **Capstone/Practicum** (4 cr) — portfolio + new project

### Key Features
✅ **AI-First:** Every course integrates AI tools (Claude, ChatGPT, Copilot)
✅ **Project-Based:** 2-3 major projects per course, no traditional exams
✅ **Python + Jupyter:** Universal computational backbone
✅ **Studio Sessions:** Weekly live project-focused sessions with faculty
✅ **GitHub Integration:** All student code on GitHub (version control habit)
✅ **Stackable Pathways:**
   - Core Analytics Certificate (12 mo, 12 cr)
   - AI Analytics Certificate (18 mo, 18 cr)
   - Full Master's (24-36 mo, 36 cr)

---

## 📅 Development Timeline

### Phase 1: Content Development (Jan-Jun 2026)
- Course syllabi finalization (Mar 1 deadline)
- Jupyter Book creation (8 chapters per course)
- Video lecture production (60-80 videos)
- Project specification + rubrics
- Studio session planning
- GitHub org setup

**Deliverables:** All course materials, videos, assessment rubrics

### Phase 2: Technology Infrastructure (Jan-May 2026)
- JupyterHub cloud hosting
- AWS student credits + setup
- Tableau licensing
- Canvas LMS configuration
- Video hosting (YouTube)

**Deliverables:** All systems tested and documented

### Phase 3: Marketing & Recruitment (Feb-Aug 2026)
- Marketing materials + website
- Digital ads ($30K budget)
- Recruitment events
- Target: 50-75 students enrolled by Aug 1

### Phase 4: Pilot & Refinement (May-Jul 2026)
- Internal pilot cohort (10-15 people)
- Feedback collection + analysis
- Content refinement
- Final QA

### Phase 5: Launch (Aug 2026+)
- **Aug 25:** Fall 1 Launch (BADM 554, BDI 513 Part 1)
- **Oct 1:** Fall 2 Launch (FIN 550, BDI 513 Part 2)
- **Jan 2027:** Spring 1 Launch (BADM 557, electives)
- Continue through Fall 4 (Oct 2027)

---

## 👥 Team & Responsibilities

### Faculty (5-6)
- Teach core courses
- Develop course materials
- Lead studio sessions + office hours
- Mentor students

### Instructional Designer (1)
- Oversee curriculum design
- Coordinate Jupyter Book creation
- Assess course quality + consistency
- Support faculty

### Video Producer (1)
- Record + edit lectures
- Add closed captions
- Manage video hosting

### Tech Support (1)
- Manage cloud infrastructure
- Student technical support
- Monitor systems + costs

### Program Manager (1)
- Overall coordination
- Timeline + budget tracking
- Vendor management
- Student communications

### Teaching Assistants (3-5)
- Office hours support
- Forum moderation
- Grading + feedback
- Peer mentoring

### Career Services (1)
- Resume review
- Interview coaching
- Job placement support
- Alumni networking

---

## 📊 Success Metrics (Year 1)

### Enrollment & Completion
- ✅ 50+ students enroll Fall 1
- ✅ 90%+ course completion rate
- ✅ 70%+ student satisfaction (NPS >70)

### Learning Outcomes
- ✅ All students use Python + Jupyter + GitHub
- ✅ All students complete 2-3 major projects per course
- ✅ 100% of core courses launch on-time

### Community
- ✅ 70%+ studio session attendance (live or recorded)
- ✅ 50%+ Discord engagement
- ✅ 75%+ sense of community belonging

### Career Outcomes
- ✅ 70%+ employed within 6 months
- ✅ Average $15K+ salary increase post-grad
- ✅ 90%+ would recommend to friend

---

## 🚀 Getting Started

### For Faculty

1. **Read first:** [Program Overview (CLAUDE.md)](CLAUDE.md)
2. **Understand strategy:** [AI-First Strategy](MSBAI_AI_FIRST_STRATEGY.md) + [Guiding Principles](MSBAI_GUIDING_PRINCIPLES_AND_CONSTRAINTS.md)
3. **Your course:** Review [Course Adaptations](COURSE_ADAPTATIONS.md) for your assigned course
4. **Curriculum questions:** See [Implementation Plan](IMPLEMENTATION_PLAN_AND_TASK_BREAKDOWN.md) Phase 1.1

**Next Step:** Attend faculty kickoff meeting (schedule TBD)

### For Program Leadership

1. **Start here:** [Implementation Plan](IMPLEMENTATION_PLAN_AND_TASK_BREAKDOWN.md) (full roadmap + resource allocation)
2. **Budget:** See Phase 1-2 budget breakdown (~$550K year 1)
3. **Timeline:** Critical milestones on page 2 of implementation plan
4. **Enrollment:** See Phase 3 & recruitment strategy

**Next Step:** Assign faculty, hire instructional designer, begin content development

### For Instructional Designers

1. **Program context:** [CLAUDE.md](CLAUDE.md) + [AI-First Strategy](MSBAI_AI_FIRST_STRATEGY.md)
2. **Your assignments:**
   - Jupyter Book structure + guidelines
   - Grading rubrics + assessment consistency
   - Quality review of faculty-created content
3. **Detailed specs:** [Course Adaptations](COURSE_ADAPTATIONS.md)

**Next Step:** Meet with faculty to discuss Jupyter Book standards

### For Admissions & Student Services

1. **Program overview:** [CLAUDE.md](CLAUDE.md) + [AI-First Strategy](MSBAI_AI_FIRST_STRATEGY.md)
2. **Student experience:** [Cohort Model](COHORT_MODEL.md) (support structure, community, onboarding)
3. **Career outcomes:** [Implementation Plan](IMPLEMENTATION_PLAN_AND_TASK_BREAKDOWN.md) Phase 3 (placement targets)

**Next Step:** Develop marketing materials + recruitment strategy

---

## 📚 Complete Document Index

### Strategic Documents
- [Program Overview & Roadmap](CLAUDE.md)
- [AI-First Strategy & Market Positioning](MSBAI_AI_FIRST_STRATEGY.md)
- [Guiding Principles & Constraints](MSBAI_GUIDING_PRINCIPLES_AND_CONSTRAINTS.md)
- [5-Phase Implementation Plan & Task Breakdown](IMPLEMENTATION_PLAN_AND_TASK_BREAKDOWN.md)

### Curriculum Documents
- [Online Curriculum Structure (8-Week Format)](ONLINE_CURRICULUM_STRUCTURE.md)
- [Detailed Course Adaptations (All 6 Courses)](COURSE_ADAPTATIONS.md)
- [Electives Roadmap & Specialization Tracks](ELECTIVES_ROADMAP.md)
- [Cohort Model & Student Experience](COHORT_MODEL.md)

---

## ❓ FAQs

**Q: Can students do this part-time?**
A: Yes. 8-week courses allow taking 1-2 courses per term while working. Most students take 2 courses/term (12 credits/year) → 3-year path. Some accelerate to 4 courses/term → 24-month path.

**Q: What if I miss a studio session?**
A: No penalty. All sessions recorded + uploaded within 24 hours. Async-first philosophy means you can catch up on your schedule.

**Q: Is AI tool use (Claude, ChatGPT) encouraged?**
A: Yes. We teach responsible AI usage. Students learn when to use AI vs. do manual work. AI is integrated into every course as a productivity tool.

**Q: Will there be real-time exams?**
A: No. All assessment is project-based. Projects demonstrate competency better than exams. No proctored exams.

**Q: How much time should I expect to spend per week?**
A: 15-20 hours/week on average. Includes: lectures (4-5 hrs), projects (6-8 hrs), studio sessions (1.5-2 hrs), readings/assignments (3-4 hrs). Intensive but doable for working professionals.

**Q: Will I get internships or job placement help?**
A: Yes. Dedicated career services staff: resume review, interview coaching, job board, employer recruiting events, alumni networking.

**Q: What's the capstone?**
A: Two-part capstone in Fall 4:
- Part 1 (Weeks 1-2): Portfolio curation + GitHub portfolio assembly
- Part 2 (Weeks 3-8): New individual project (real client or independent research with AI assist)

**Q: Are there scholarships?**
A: Yes. Need-based aid + merit scholarships available. Goal: 20% cheaper than peer programs. Active recruitment of underrepresented groups.

---

## 📞 Contact

**Program Director:** [TBD]
**Email:** [msba-online@illinois.edu] (TBD)
**GitHub Repo:** [github.com/vishalsachdev/msba-online](https://github.com/vishalsachdev/msba-online)

---

## 📄 License & Attribution

**Documentation prepared:** January 12, 2026
**Lead:** Claude Code (Anthropic Claude AI)
**Status:** Ready for faculty course development & implementation

All course materials, syllabi, and curriculum designs are proprietary to Gies College of Business, University of Illinois. Internal use only.

---

*Last Updated: January 12, 2026*
*Next: Faculty assignments + content development kickoff*
